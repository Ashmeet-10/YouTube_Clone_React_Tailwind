...
const PlaylistVideos = ({updateId}) => {
  ...
  return (
    <div className='bg-[#0f0f0f] max-w-[2800px] mx-auto text-white space-y-4 px-4 pb-4 xs:px-1'>
      {playlistsInfo.meta && <div className="relative flex flex-col pt-8 mb-6 sm:flex-row lg:justify-center lg:space-x-6">
        <img src={`${playlistsInfo.data[0].thumbnail[playlistsInfo.data[0].thumbnail.length-1].url}`} className="absolute w-full h-full blur-2xl shadow-sm bg-cover z-0" alt="" />
        <img src={`${playlistsInfo.data[0].thumbnail[playlistsInfo.data[0].thumbnail.length-1].url}`} className="w-4/5 mx-auto rounded-xl z-10 shadow-lg sm:w-2/5 sm:h-min lg:w-[24rem] sm:mt-6 lg:mx-0 min-[2100px]:w-[450px]" alt="" />
      ...
      <div className="flex flex-col xs:grid xs:grid-cols-2 min-[860px]:grid-cols-3 lg:grid-cols-4 lg:gap-2 3xl:grid-cols-5 min-[2600px]:grid-cols-6">
